...
- **신원과 파일:** 새 영속 쓰기는 현재 확인된 subject가 있을 때만 허용합니다. 이메일만 확인된 경우 메모리에서 처리합니다. 파일은 0600 권한으로 원자 교체하며 해시는 결합 키이지 암호화나 익명화가 아닙니다. 세대·작업 식별자를 확인하여 폐기된 비동기 쓰기를 거절합니다. 로그아웃하면 현재 계정 파일 삭제를 시도하지만 최선 노력 방식이며 안전한 완전 삭제를 보장하지 않습니다. OS가 쓰기나 삭제를 거부하면 이전 파일이 남을 수 있습니다.
