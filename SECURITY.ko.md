...
- **금액:** `~/Library/Application Support/CursorMeter/cycle-usage-v1.json`에 최대 1 MiB인 주기별 집계 스냅샷 하나를 저장합니다. 기준 사용률·금액 합계·수집 범위·추정 근거·시각·해시된 계정/범위 식별자를 포함하며, 원시 이벤트·이름·이메일·쿠키·대화 내용은 저장하지 않습니다. 동일하게 확인된 subject·개인 범위·플랜·진행 중인 주기가 일치하고 24시간 이내의 스냅샷일 때만 복원합니다. 복원 데이터는 점프 기준이나 알림 이력을 초기화하지 않습니다.
- **알림 원장:** `~/Library/Application Support/CursorMeter/SplitAlerts/`에 성공한 임계치 알림의 해시 식별자와 주기 종료일을 저장하며, 계정 파일당 최대 4,096건·1 MiB입니다. 현재 주기는 주기 내내 유지하고 이전 주기는 종료 7일 후 만료합니다. 실패한 전송은 기록하지 않으며 Bold 점프는 영속 저장하지 않습니다.
- **신원과 파일:** 새 영속 쓰기는 현재 확인된 subject가 있을 때만 허용합니다. 이메일만 확인된 경우 메모리에서 처리합니다. 파일은 0600 권한으로 원자 교체하며 해시는 결합 키이지 암호화나 익명화가 아닙니다. 세대·작업 식별자를 확인하여 폐기된 비동기 쓰기를 거절합니다. 로그아웃하면 확인된 현재 계정의 파일 삭제를 시도합니다. 오프라인 상태로 새로 실행하여 확인된 subject가 없으면 이전 계정 파일의 소유자를 식별할 수 없으므로, 해당 파일을 복원하지 않으며 소유권을 추측해 삭제하지도 않습니다. 삭제는 최선 노력 방식이며 안전한 완전 삭제를 보장하지 않습니다. OS가 쓰기나 삭제를 거부하면 이전 파일이 남을 수 있습니다.
- **요청:** 한도가 있는 월간 수집은 별도 DTO를 사용하고 원시 응답 본문을 저장하지 않습니다. 보충 조회 실패는 자격증명을 무효화하지 않으며 기존 주요 인증 응답만 해당 판단에 참여합니다.
