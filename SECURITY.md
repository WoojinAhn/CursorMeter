...
- **Identity and files:** only a freshly verified subject enables new persistent writes. Email-only identity remains in memory. Files are atomically replaced with mode 0600; hashes are binding keys, not encryption or anonymization. Generation/operation checks reject retired asynchronous writes. Logout attempts to remove the verified current account's files. A cold offline start without a verified subject cannot identify old account files; those remain unread and are not deleted by guessing ownership. Deletion is best effort, not secure erasure, and OS-level write/removal failures can leave an older file on disk.
