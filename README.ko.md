...
- 사용량 임계치 도달 시 macOS 알림(기본값: 80%/90%, 조정 가능). 분리 플랜은 Cursor Models·Other Models·유료 예산 대상을 각각 선택할 수 있습니다.
...
- **주기 요약** — 설정 → Usage에서 공식 사용률, 관측된 달러 활동, 조건부 추정 한도를 구분합니다. 금액에는 별도 기준 시각과 수집 범위를 표시하며 추정 한도는 보장된 청구 한도가 아닙니다. 추가 지출과 Bot 활동은 별도로 유지합니다. 월간 수집은 한도를 두며 기본 미터 갱신을 지연시키지 않습니다.
- **최근 사용 내역** — 설정 → Usage → Recent에서 최근 요청 최대 30건의 모델·시각·유형·토큰·달러 금액을 확인할 수 있습니다. Included 금액은 추가 청구액이 아니라 요금제에 포함된 사용 가치입니다. **Local**(기본값: 이 Mac의 시간대)과 **UTC**를 선택할 수 있으며, **Open Cursor**로 전체 내역과 청구 대시보드를 엽니다.
...
- `URLSessionConfiguration.ephemeral` (HTTP 디스크 캐시 없음); 제한된 최근·주기 스냅샷 및 성공한 알림 원장은 별도 저장
- Keychain 기반 인증 정보 저장
...
<p align="center"><em>단일 미터 화면의 예시 데이터입니다. 분리 플랜은 <a href="docs/mockup-121-app.html">앱 전체 시안</a>에서 확인할 수 있으며 실제 캡처는 수동 검증 후 갱신할 예정입니다. 이미지를 클릭하면 원본 크기로 볼 수 있습니다.</em></p>
